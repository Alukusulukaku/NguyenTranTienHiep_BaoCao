    ...
    function () {
      const delayDebounceFn = setTimeout(() => {
        async function fetchItems() {
          ...
          const result2 = await productservice.getById(id);
          setName(result2.data.data.name);
          setMetakey(result2.data.data.metakey);
          setMetadesc(result2.data.data.metadesc);
          setDescription(result2.data.data.description);
          setPrice(result2.data.data.price);
          setCategory(result2.data.data.category_id);
          setBrand(result2.data.data.brand_id);
          setStatus(result2.data.data.status);
        }
        fetchItems();
      }, 1000);
