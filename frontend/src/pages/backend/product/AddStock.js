...
import StockProduct from "../../../components/backend/StockProduct";
...
function AddStock() {
  ...
  const addToSelectedProducts = () => {
    if (parseInt(qty) === 0) {
      setAlert({
        text: "Vui lòng nhập số lượng muốn nhập kho",
        type: "failed",
      });
      return;
    }
    const existingProductIndex = selected_products.findIndex(
      (product1) => product1.id === product.id
    );

    if (existingProductIndex !== -1) {
      const updatedProducts = [...selected_products];
      const existingProduct = selected_products[existingProductIndex];
      ...
      setAlert({
        text: "Sản phẩm đã tồn tại ở danh sách phía dưới và đã được cập nhật số lượng và giá",
        type: "success",
      });
    } else {
      const newProduct = {
        ...
        async function fetchItems() {
          if (product_id !== 0) {
            ...
            if (result.data.data.productstore != null) {
              setPrice(result.data.data.productstore.price);
            } else {
              setPrice(0);
            }
          }
  ...
  async function productAddSale(event) {
    ...
    await productservice.addStock(product).then(function (res) {
