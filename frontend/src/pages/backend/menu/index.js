...
function Menu() {
  ...
  const newBtn = () => {
    let path = "/admin/menu/new";
    navigate(path);
  };
  const [deleteId] = useContext(DeleteContext);
  const [menus, setMenus] = useState([]);
  const [total_menus, setTotal_Menus] = useState([]);
    ...
    function () {
      ...
      const delayDebounceFn = setTimeout(() => {
        async function fetchItems() {
          const result = await menuservice.getAll(limit, page);
          setMenus(result.data.data);
          setTotal_Menus(result.data.total_data);
        }
        fetchItems();
      }, 1000);
          ...
          return (
            ...
            >
              ...
              <td className="tw-px-6 tw-py-4 tw-flex tw-items-center tw-justify-end">
                <Threedot id={item.id} table={`menu`} />
              </td>
            </tr>
          );
        })}
    ...
    <>
      ...
      <div className="tw-relative tw-overflow-x-auto tw-shadow-md sm:tw-rounded-lg">
        <table className="tw-w-full tw-text-sm tw-text-left tw-text-gray-500 light:tw-text-gray-400">
          ...
          <tbody>{menus.length > 0 ? result() : skeleton()}</tbody>
        </table>
      </div>
      <div className="tw-flex tw-flex-col tw-items-center tw-mt-5">
        <span className="tw-text-sm tw-text-gray-700 light:tw-text-gray-400">
          ...
          <span className="tw-font-semibold tw-text-gray-900 light:tw-text-white">
            {(() => {
              return (parseInt(page) - 1) * limit + menus.length;
            })()}
          </span>{" "}
          of{" "}
          <span className="tw-font-semibold tw-text-gray-900 light:tw-text-white">
        ...
        <div className="tw-flex tw-mt-2 tw-space-x-2 xs:tw-mt-0">
            ...
            if (parseInt(page) === 1)
              return (
                <button
                  ...
                  disabled
                  onClick={() => navigate(`/admin/menu/${parseInt(page) - 1}`)}
                >
                  Prev
                </button>
              );
          ...
          {(() => {
            if (
              (parseInt(page) - 1) * limit + menus.length ===
              total_menus.length
            )
              return (
                <button
